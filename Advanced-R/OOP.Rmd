...
An S3 object is an instance of some class ad has optional attributes
...
Best practices is to provide three functions:
  1. A constructor to ensure that an object is structurally correct (Ensure consistent nameing of fields for example)
...
newStudent2 <- function(num,nam,age,gpa) {
  ...
  if(!studentValidator(age,gpa)) {
    stop("Invalid dats supplied for student object", call. = TRUE)
  }
...
This means we can have a collection of objects, each of which has a different implementatation of a method
